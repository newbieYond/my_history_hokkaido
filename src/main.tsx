...
const days: Day[] = [
  ...
  { date: "11.21 SAT", title: "푸른 운하, 오타루", area: "OTARU", flow: "미나미오타루 · 사카이마치 · 운하 · 테미야선", food: "초밥 · 르타오 · 오뎅", tone: "otaru", detail: "JR로 미나미오타루역에서 시작해 다나카주조, 사카이마치, 르타오를 지나 오타루 운하까지 내리막으로 걷는다. 운하 플라자와 아트 베이스를 선택지로 두고, 일몰 뒤 푸른 운하·구 테미야선 산책로와 하츠하나를 잇는다. 초밥·닭튀김·운하 식당가는 식사 대안으로 남긴다.", status: "JR 당일치기" },
  ...
  { date: "11.24 TUE", title: "공항의 마지막 한입", area: "NEW CHITOSE", flow: "체크아웃 · 국내선 2층 · 연결시설 · 귀국", food: "에비소바 · 소프트아이스크림", tone: "airport", detail: "10:00 체크아웃 후 공항으로 바로 이동. 국내선 2층에서 기념품을 사고, 라멘 도장에서 점심을 해결한 뒤 스마일 로드를 따라 로이즈 초콜릿 월드와 도라에몽 스카이파크를 선택한다. 누벨바그 르타오·모리모토는 개인 선물 보완용이다. 7C1504 16:00 출발 · 19:25 인천 도착.", status: "항공 확정" }
];
...
function App() {
  ...
  return <main className="app-shell">
    ...
    <div className="screen-content">
      ...
      {activeScreen === "saved" && <section className="saved-places"><div className="saved-title"><p className="section-label">SAVED ON MAPS</p><h2>저장해 둔<br /><em>{places.length}개의 장소.</em></h2><p>Google 지도 Takeout에서 가져온 원본 링크와, 함께 고른 일정 추가 장소입니다. 각 장소를 눌러 바로 지도에서 열 수 있어요.</p><p className="place-verification">2026.09.05 · Google 지도 확인<br />폐업·일시 휴업 표기 0곳</p></div><div className="saved-content"><div className="place-tabs" role="tablist" aria-label="저장 장소 권역"><div className="place-tabs-scroll">{savedPlaceTabs.map((tab, index) => <button type="button" role="tab" aria-selected={index === activePlaceTab} className={index === activePlaceTab ? "active" : ""} onClick={() => setActivePlaceTab(index)} key={tab.label}>{tab.label}<span>{tab.places.length}</span></button>)}</div></div><article className="saved-group" role="tabpanel"><div className="saved-area"><h3>{activeSavedPlaceGroup.label}</h3><span>{activeSavedPlaceGroup.places.length} PLACES</span></div><ul>{activeSavedPlaceGroup.places.map(place => <li key={place.id}><a href={place.googleMapsUrl} target="_blank" rel="noreferrer"><span>{place.name}</span><small>{place.isSelected ? `일정 추가 · ${categoryLabel[place.category]}` : categoryLabel[place.category]}</small><b>↗</b></a></li>)}</ul></article></div></section>}
