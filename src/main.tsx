...
function App() {
  ...
  return <main className="app-shell">
    ...
    <div className="screen-content">
      ...
      {activeScreen === "saved" && <section className="saved-places"><div className="saved-title"><p className="section-label">SAVED ON MAPS</p><h2>저장해 둔<br /><em>{places.length}개의 장소.</em></h2><p>Google 지도 Takeout에서 가져온 원본 링크와, 동선에 맞춰 추가한 검토 후보입니다. 각 장소를 눌러 바로 지도에서 열 수 있어요.</p></div><div className="saved-content"><div className="place-tabs" role="tablist" aria-label="저장 장소 권역"><div className="place-tabs-scroll">{savedPlaceTabs.map((tab, index) => <button type="button" role="tab" aria-selected={index === activePlaceTab} className={index === activePlaceTab ? "active" : ""} onClick={() => setActivePlaceTab(index)} key={tab.label}>{tab.label}<span>{tab.places.length}</span></button>)}</div></div><article className="saved-group" role="tabpanel"><div className="saved-area"><h3>{activeSavedPlaceGroup.label}</h3><span>{activeSavedPlaceGroup.places.length} PLACES</span></div><ul>{activeSavedPlaceGroup.places.map(place => <li key={place.id}><a href={place.googleMapsUrl} target="_blank" rel="noreferrer"><span>{place.name}</span><small>{place.isCandidate ? `검토 · ${categoryLabel[place.category]}` : categoryLabel[place.category]}</small><b>↗</b></a></li>)}</ul></article></div></section>}
